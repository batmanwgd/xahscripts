...
"
  ...
  ;     (xah-delete-xahtemp-files $destDir)
    ...
    (mapc
     (lambda ($oneSrcDir)
       (let (($fileList
              (directory-files-recursively (concat $destDir (string-remove-prefix $root $oneSrcDir)) "\\.html$" )
              ;; (find-lisp-find-files
              ;;  (concat $destDir (string-remove-prefix $root $oneSrcDir)) "\\.html$")
              ))
         (mapc
          (lambda (x)
            (let (($origPath (concat $root (substring x (length $destDir)))))
              ;; (progn
              ;;   (princ "\n70556---------------\n")
              ;;   (princ x)
              ;;   (princ "\n")
              ...
              ;;   (princ "\n")
              ;;   (princ $root))
              (xah-process-file-for-download x $origPath $root)))
          $fileList)))
     $sourceDirList)
