...
# check if every item in listA is in listB, and vice versa. that is, they should be the same.
...
def myProcessFile(fp):
    ...
    xntList = content.split(r'''class="xnt">''')
    xntList.pop(0);
    ...
    for seg in xntList:
        ...
        if found:
            ...
            keyList.append(keyphrase)

    x3ntList = content.split(r'''class="x3nt">''')
    x3ntList.pop(0);
    ...
    if keyList != targetList:
        print("{}\n".format(fp))
        print (keyList)
        print (targetList)

        insex = set(keyList).intersection( set(targetList) )

        keySetDiff = set(keyList) - insex
        targetSetDiff = set(targetList) - insex

        print( "「{}」\n「{}」\n".format(keySetDiff, targetSetDiff))
    
    # print(keySetDiff)
    # print(targetSetDiff)
    # for i,v in enumerate(keyList):
    #     if v != targetList[i]:
    #         print( "problem: 「{}」 「{}」 {}".format(v, targetList[i], fp ))
    #     break
